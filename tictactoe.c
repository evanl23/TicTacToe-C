...
for (r = BOARD_START_ROW; r < BOARD_ROWS; r++) {
	if (Board_scan_line_for_marker(r, BOARD_START_COL, r, BOARD_END_COL, BOARD_SCAN_DIR_DOWN, BOARD_SCAN_DIR_NONE, m)) {return true;}
}
				...
				int horizontal_dir, int vertical_dir, char m)
{
	if (r == end_row && c == end_col) {
		...
		return (Board[r][c] == m) && Board_scan_line_for_marker(r+vertical_dir, c+horizontal_dir, end_row, end_col, horizontal_dir, vertical_dir, m);
	}
}
